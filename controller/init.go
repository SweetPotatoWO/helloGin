...
//格式 为 User/addUser
func (cInit *InitController) splitControllerAndFnName(controllerFnCode) (map[string]string,error) {
	return nil,nil
}

//在调用某个子类之前会提前调用 可在这里写一些通用的逻辑
//适用于全部的基础类 必须返回true 只有返回true的情况才会继续运行
func (cInit *InitController) initBeforeFn() bool {
	return true
}

//在调用某个子类之前会提前调用 可在这里写一些通用的逻辑
//适用于全部的基础类 必须返回true 只有返回true的情况才会继续运行
func (cInit *InitController) initAfterFn() bool {
	return true
}
